...
void LaunchSoftware() { //If dolphin, restarts game, else launches Riivo->HBC->OHBC->WiiMenu
    ...
    result = IO::OpenFix("/title/00010001/43545243/content/title.tmd\0", IOS::MODE_NONE); //Classic Wii
    if(result >= 0) {
        ISFS::Close(result);
        OS::LaunchTitle(0x00010001, 0x43545243);
        return;
    }
    result = IO::OpenFix("/title/00010001/57435443/content/title.tmd\0", IOS::MODE_NONE); //Classic vWii
    if(result >= 0) {
        ISFS::Close(result);
        OS::LaunchTitle(0x00010001, 0x57435443);
        return;
    }
