...
static int MiiHeads(RaceData* racedata, u32 unused, u32 unused2, u8 id) {
    ...
    if(gamemode == MODE_TIME_TRIAL || roomType == RKNet::ROOMTYPE_FROOM_HOST || roomType == RKNet::ROOMTYPE_FROOM_NONHOST) charId = racedata->racesScenario.players[id].characterId;
    }
    return charId;
}
...
kmWrite32(0x8055422C, 0x48000044);
 ...
 void FPSPatch() {
    FPSPatchHook = 0x00;
    if (Pulsar::Settings::Mgr::GetSettingValue(Pulsar::Settings::SETTINGSTYPE_MENU2,SETTINGMENU_RADIO_FPS) == 1) {
